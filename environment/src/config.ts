...
const configSchema = z.object({
  ...
  gridWidth: z.coerce.number().int().min(2).default(8),
  gridHeight: z.coerce.number().int().min(2).default(8),
  maxSteps: z.coerce.number().int().positive().default(100),
  robotStart: z.string().default('0,0').transform((value) => parsePosition(value, 'robotStart')),
  boxStart: z.string().default('2,2').transform((value) => parsePosition(value, 'boxStart')),
  goalPosition: z.string().default('7,7').transform((value) => parsePosition(value, 'goalPosition')),
